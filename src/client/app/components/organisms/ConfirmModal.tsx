...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-xs z-[100]">
          <motion.div
          ...
          >
            ...
            <div className="flex justify-end space-x-4 mt-6">
              <button
                className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 flex items-center font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={onCancel}
                disabled={isLoading}
              >
              ...
              <button
                className={`px-4 py-2 text-white rounded-md flex items-center font-medium transition-colors ${confirmButton} ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
                onClick={onConfirm}
                disabled={isLoading}
              >
                {isLoading ? (
