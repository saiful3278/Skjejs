...
async function fetchCategoryProductsDirect(categoryName) {
  const env = window.__ENV || {};
  const base = env.SUPABASE_URL || '';
  const key = env.SUPABASE_ANON_KEY || '';
  if (!base || !key) return [];
  const url = `${base}/rest/v1/products?select=id,name,price,image_url,description&category=eq.${encodeURIComponent(categoryName)}&order=created_at.desc`;
  try {
...
async function loadAllCategoryProducts(categoryName) {
  const client = window.supabaseClient;
  if (!client) {
    const direct = await fetchCategoryProductsDirect(categoryName);
...
function init() {
  ...
  if (window.supabaseClient) {
    ...
    loadAllCategoryProducts(categoryName);
  } else {
  ...
  if (searchBtnEl && searchInputEl) {
    searchBtnEl.addEventListener('click', async () => {
      const gridEl = document.querySelector('.product-grid');
      if (gridEl) gridEl.innerHTML = '<div class="loading"></div>';
