...
function rowHtml(p) {
  ...
  const displayCat = !cat || cat === 'REVIEW_NEEDED' ? 'UNCATEGORIZED' : cat;
  return `<tr>
...
async function loadProducts() {
  ...
  tableBody.innerHTML = '<tr><td colspan="8"><span class="loading"></span></td></tr>';
  let all = [];
  let from = 0;
  const pageSize = 1000;
  while (true) {
    ...
    if (currentCategoryFilter) {
      if (currentCategoryFilter === 'UNCATEGORIZED') {
        q = q.or('category.is.null,category.eq.UNCATEGORIZED,category.eq.,category.eq.REVIEW_NEEDED');
      } else {
  ...
  if (listTotalEl) listTotalEl.textContent = allProducts.length;
...
function renderProductsTablePage(page) {
  ...
  if (pageInfoEl) {
    const totalPagesDisplay = total ? totalPages : 1;
    pageInfoEl.textContent = `Page ${total ? page : 0} / ${totalPagesDisplay}`;
  }
  if (prevBtn) prevBtn.disabled = !total || page <= 1;
  if (nextBtn) nextBtn.disabled = !total || page >= totalPages;
}
...
async function loadFilterCategories() {
  ...
  if (Array.isArray(names)) {
    names
      .filter((cat) => String(cat).trim().toUpperCase() !== 'REVIEW_NEEDED')
      .forEach((cat) => {
...
if (btnBulkAuto) {
  btnBulkAuto.addEventListener('click', async () => {
    ...
    if (autoDetectStatusEl) autoDetectStatusEl.textContent = `Detecting... 0 / ${ids.length}`;
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      if (error) {
        ...
        if (autoDetectStatusEl) autoDetectStatusEl.textContent = 'Load failed';
        return alert('Load failed: ' + error.message);
      }
      ...
      if (!targets.length) {
        if (autoDetectStatusEl) autoDetectStatusEl.textContent = `Detecting... ${Math.min(scannedCount, ids.length)} / ${ids.length} | updated ${updatedCount}`;
        continue;
      }
      const groups = {};
      for (const p of targets) {
        ...
        const rawCat = autoCategory(title, desc, cats);
      ...
      for (const cat in groups) {
        ...
        for (let j = 0; j < idsForCat.length; j += UPDATE_BATCH) {
          ...
          if (!uErr) updatedCount += sub.length;
        }
      }
      if (autoDetectStatusEl) autoDetectStatusEl.textContent = `Detecting... ${Math.min(scannedCount, ids.length)} / ${ids.length} | updated ${updatedCount}`;
    }
    btnBulkAuto.textContent = 'Auto Detect';
    removeButtonSpinner(btnBulkAuto);
    if (autoDetectStatusEl) autoDetectStatusEl.textContent = `Done: updated ${updatedCount}`;
    if (updatedCount > 0) {
      alert(`Updated ${updatedCount} products!`);
      loadProducts();
      if (bulkToolbar) bulkToolbar.style.display = 'none';
